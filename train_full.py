...
""",
    ...
    parser.add_argument(
        "--modality", required=True, choices=["image", "video", "audio"],
    ...
    if not args.skip_download:
        ...
        run_cmd(
            [sys.executable, "prepare.py", "--verify", "--modality", args.modality],
            "Verifying cache",
        )
    ...
    if proc.returncode == 0:
        ...
        run_cmd(eval_cmd, "Evaluating trained model", check=False)
    ...
    if proc.returncode == 0 and not args.skip_export:
        ...
        if model_name is None:
            ...
            defaults = {"image": "efficientnet-b4", "video": "r3d-18", "audio": "wav2vec2"}
